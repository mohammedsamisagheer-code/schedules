  ...
  try {
    ...
    if (json.ok) {
      ...
      document.getElementById("conflictMsg").textContent =
        "الأستاذ لديه حصة أخرى في هذا الوقت. لا يمكن نقل الحصة هنا.";
...
document.addEventListener("DOMContentLoaded", () => {
  if (
    ...
    sessionStorage.getItem("scheduleEditMode") === "1"
  ) {
    toggleEditMode();
  }
  ...
  document.addEventListener("drop", (e) => {
    ...
    if (dragState.day === targetDay && dragState.time === targetTime) {
      dragState = null;
      return;
    }
    ...
    if (checkAdjacentConflict(draggedId, draggedTerm, targetDay, targetTime)) {
      const t = parseInt(draggedTerm);
      const adjTerms = [t - 1, t + 1].filter(
        (x) => x >= minTerm && x <= maxTerm,
      );
      const termLabels = adjTerms.map((x) => "الفصل " + x).join(" و ");
      document.getElementById("conflictMsg").textContent =
        "توجد حصة في نفس الوقت في " +
        termLabels +
...
async function exportToExcel() {
  fetch("log_action.php", {
    ...
    body: "action=تصدير الجدول العام Excel",
  });
  const colorMap = {
  ...
  function styleCell(cell, opts) {
    ...
    cell.font = {
      bold: !!bold,
      color: { argb: "FF" + (fontColor || "000000") },
      name: "Cairo",
      size: 10,
    };
    cell.border = {
      ...
      left: {
        style: borderColor ? "medium" : "thin",
        color: { argb: "FF" + (borderColor || "E5E7EB") },
      },
  ...
  if (selectedTerm === "all") {
    const sheet = workbook.addWorksheet("الجدول العام", {
    ...
    for (const term of availableTerms) {
      ...
      slotKeys.forEach((slotKey, si) => {
        ...
        days.forEach((day, di) => {
          ...
          const entries =
            (lookup[term] && lookup[term][day] && lookup[term][day][slotKey]) ||
            [];
          if (entries.length > 0) {
            const e = entries[0];
    ...
    titleCellSingle.value =
      "جدول محاضرات " + sheetName + " لفصل " + academicYear;
    titleCellSingle.alignment = {
      vertical: "middle",
      horizontal: "center",
      readingOrder: 2,
    };
    titleCellSingle.fill = {
      type: "pattern",
      pattern: "solid",
      fgColor: { argb: "FF595959" },
    };
    titleCellSingle.font = {
      bold: true,
      color: { argb: "FFFFFFFF" },
      name: "Cairo",
      size: 13,
