...
<body class="font-sans antialiased">
    ...
    <main class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
                ...
                <table id="scheduleZoomTable" class="w-full text-right border-collapse min-w-[800px]">
                    <thead>
                        <tr class="bg-gray-50">
                            <?php if ($show_all): ?>
                                <th
                                    class="p-4 text-xs font-semibold text-gray-500 uppercase tracking-wider text-center w-[80px]">
                                    الفصل</th>
                            <?php endif; ?>
                            <th
                                class="p-4 text-xs font-semibold text-gray-500 uppercase tracking-wider text-center w-[120px]">
                                الوقت</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <?php if ($show_all): ?>
                            <?php foreach ($available_terms as $term): ?>
                                <?php $slot_count = count($time_slots);
                                $first = true; ?>
                                <?php foreach ($time_slots as $slot_raw => $slot_label): ?>
                                    <?php $slot_key = date('H:i', strtotime($slot_raw)); ?>
                                    <tr>
                                        <?php if ($first): ?>
                                            <td class="bg-primary/5 p-2 text-center font-bold text-primary text-sm border-r border-gray-100"
                                                rowspan="<?php echo $slot_count; ?>">
                                                <?php echo htmlspecialchars($term_names[$term] ?? 'الفصل ' . $term); ?>
                                            </td>
                                            <?php $first = false; endif; ?>
                                        <td class="bg-gray-50/50 p-4 text-center">
                                            <div
                                                class="flex flex-col items-center justify-center text-xs text-gray-500 font-medium">
                                            ...
                                            <td class="day-col p-2 border-r border-gray-100">
                                                <?php if (isset($schedules_by_term_day_time[$term][$day][$slot_key])): ?>
                                                    <?php foreach ($schedules_by_term_day_time[$term][$day][$slot_key] as $schedule): ?>
                                                        ...
                                                        <div
                                                            class="class-card <?php echo $colors['bg']; ?> border-r-4 <?php echo $colors['border']; ?> p-3 rounded flex flex-col justify-between">
                                                            <div>
                                                ...
                                                <?php else: ?>
                                                    <div
                                                        class="class-card bg-gray-100 border-r-4 border-gray-400 p-3 rounded flex items-center justify-center">
        ...
        <div class="flex items-center justify-center gap-4 mt-4 pb-2 no-print" id="zoomControls">
            <button type="button" onclick="zoomOut()"
                class="w-9 h-9 flex items-center justify-center bg-white border border-gray-200 rounded-full text-gray-600 hover:bg-gray-50 shadow-sm text-xl font-bold select-none">&#x2212;</button>
    ...
    <footer class="bg-white border-t border-gray-200 py-6 mt-12">
        <div
            ...
            <p class="text-sm text-gray-400"> 2026 <?php echo htmlspecialchars(COLLEGE_NAME); ?>. جميع الحقوق محفوظة.
            </p>
